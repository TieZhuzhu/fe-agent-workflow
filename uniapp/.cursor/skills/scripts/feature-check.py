#!/usr/bin/env python3
"""Feature SDD gate — static checks + optional lint/build (uni-app).
...
BLOCKER_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("路由 path 未定义", re.compile(
        r"path\s*未定义|路由\s*未定义|pages\.json.*未注册|"
        r"path:\s*`?/?\.\.\.|path\s*:\s*[`']/?[`']",
        re.I,
    )),
    ("主接口 URL 未知", re.compile(
        r"接口\s*URL\s*未知|主列表.*未知|提交接口.*未知|接口\s*待后端|接口\s*未就绪.*无\s*URL",
        re.I,
    )),
    ("业务模块未定义", re.compile(r"模块\s*未定义|所属模块\s*未知|module\s*未定义|子包\s*未定义", re.I)),
    ("权限模型无决策", re.compile(r"权限模型.*不一致.*无决策|权限.*\[待确认\](?!-低)", re.I)),
    ("页面冲突未解决", re.compile(r"重复\s*path|重复功能|与现有页面冲突.*\[待确认\]", re.I)),
)

FILE_PATH_RE = re.compile(
    r"`((?:subPackages|pages|components|service)/[^`\s]+)`|"
    r"((?:subPackages|pages|components|service)/[\w./-]+\.(?:vue|js|ts))"
)
...
def npm_script(name: str) -> str | None:
    ...
    if name in scripts:
        return name
    aliases = {
        "lint": ("lint-fix", "lint", "lint:fix"),
        "build": ("build", "build:h5", "build:test", "build:mp-weixin"),
    }
...
def check_verify(slug: str, *, run_lint: bool, run_build: bool) -> Report:
    ...
    if run_build:
        ...
        if build_script:
            ...
            report.add("build", True, "no build script — skip", critical=False)
...
def extract_task_file_refs(tasks_content: str) -> list[str]:
    refs: set[str] = set()
    for m in re.finditer(r"`((?:subPackages|pages|components)/[^`]+)`", tasks_content):
        refs.add(m.group(1))
    for m in re.finditer(r"((?:subPackages|pages|components)/[\w./-]+)", tasks_content):
...
def check_analyze(slug: str) -> Report:
    ...
    design_has_scope = bool(re.search(r"文件清单|接口|path|pages\.json|subPackages|页面类型", design))
    ...
    has_api_in_design = bool(re.search(r"接口|services|Get|Post|url|service/", design, re.I))
    ...
    if is_project_wide(design):
        ...
        path_ok = bool(re.search(r"path|pages\.json|subPackages/|pages/", design))
        report.add("path↔目录", path_ok, "path and pages.json/subPackages planned")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Feature SDD gate checks (uni-app)")
    ...
    p_verify.add_argument("--no-lint", action="store_true", help="skip npm run lint")

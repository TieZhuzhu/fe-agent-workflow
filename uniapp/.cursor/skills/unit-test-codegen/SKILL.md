...
version: 1.0.1
description: 为 uni-app Vue 项目生成单元测试（Vue 3 Vitest 默认；Vue 2 Jest/Vitest 按 package.json）。优先 utils、toXxxParams、constants 映射、service 调用形态；不默认 E2E。用户要求写单测、补测试、test:unit 时使用。
---
# 单元测试生成（Unit Test）

> **定位：** 补强 L1 — 防止 **纯逻辑 / 参数组装** 回归；**不替代** api-smoke、feature-e2e-verify。  
> **默认策略：** 用户**明确要求**或 PR 要求时才生成；不随每次 feature-dev 自动批量产出。
...
```
common/utils/formatXxx.js
└── formatXxx.test.js

service/cart.js
└── cart.test.js       # mock request（若项目有单测约定）
```

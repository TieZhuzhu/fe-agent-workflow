...
name: feature-verify
version: 1.0.1
description: SDD 验证阶段（L1）。对照 docs/features/<slug>/ 的 spec、design、tasks、field-map 与代码，并跑 lint/test/build。可选衔接 api-smoke（L2）、feature-e2e-verify（L3）。用户说【verify】、验收 feature 时触发；【api-smoke】【verify-e2e】走对应 Skill。
---
# Feature Verify
...
Read `package.json`，按实际 scripts 执行：
...
```
Feature Verify: <slug> | PASS/FAIL | spec x/y | tasks x/y | build ✅/❌
```

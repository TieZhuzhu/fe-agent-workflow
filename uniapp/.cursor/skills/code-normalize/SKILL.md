...
name: code-normalize
version: 1.0.1
description: 按全量 rules 优化/整理现有 Vue 页面或单模块，不改业务行为。用户说「按规范优化」「规范化」「整理代码」且范围是单页/单模块时触发。全项目重构用 rules-refactor；Vue2升Vue3用 vue2-to-vue3-refactor；大文件拆分见 page-refactor。
---
# 按规范优化现有代码
...
>
> **与 page-refactor 区别：** 本 Skill 做**规范合规整理**（types、写法、目录、列表分页等）；单文件过大、多区块拆分 → `page-refactor`（可组合使用）。
>
> **与 rules-refactor 区别：** 本 Skill 范围是**单页 / 单模块**。「全项目」「所有代码」「继续重构到全部合规」→ **`rules-refactor`**（须先 `【spec】propose project-refactor`，不中途询问）。
>
...
| 列表分页、methods 整理、去 console | 单文件 >800 行大拆分 |
...
**不得**只审计 types 而跳过 Vue 写法、列表分页、样式、状态等维度。
...
- [ ] services 箭头导出
### 列表 / 表单（按页面类型）
- [ ] onReachBottom 分页；逻辑在 methods / mixins（Vue 2）或 hooks（Vue 3）
### 代码质量
...
3. **对齐同模块参照页** — 列表分页、service/ 路径、样式引入
...
- 只改 types/格式，跳过列表分页、写法、质量项却称「已优化」

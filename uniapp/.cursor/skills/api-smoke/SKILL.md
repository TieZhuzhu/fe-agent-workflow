...
name: api-smoke
version: 1.0.1
description: 接口联调实况探针（curl）。对接完成后对 test/staging 用 curl 发最小请求，核对 status、业务 code、响应结构与 field-map/OpenAPI 一致。衔接 api-integration / openapi-api-integration / feature-verify。用户说「接口探针」「HTTP 冒烟」「curl 验证」「验证响应格式」「联调验收」时触发。
---
# 接口 Smoke 探针（curl）
...
- 文档 vs 实际 keys 不一致 → **spec drift** 或 **前端 binding 错误**，给出修 types/service 建议
- 401/403 → 鉴权问题
- 5xx → 后端 blocker
...
```
API Smoke: <slug> | pass/fail/skip | 探针 2/2 | curl
```

...
version: 1.1.1
description: 基于静态 HTML 原型开发 Vue 功能。支持本地 .html 与远程 Axure 导航壳（SITEMAP）。含 JS 交互的原型：curl 批量 ingest + Browser MCP 实点 P0 路径。解析 DOM、page-spec-panel、关联 JS 与交互，映射为符合 .cursor/rules 规范的 Vue 页面。用户说「按原型开发」「静态页转 Vue」「Axure 原型链接」时触发。
---
# 静态 HTML 原型驱动的功能开发
...
```html
.list-page / .goods-list → view + v-for 列表项组件
.filter-bar / .search-bar → search / drop-down / picker
.load-more / .pagination → onReachBottom + pageNo/pageSize
```

### 列表页识别（PC 后台原型 — 须转换为移动端）

若原型含 `.admin-list-page`、`<table>`、侧栏布局，应转换为：

- 工具栏 → `view.toolbar` + `button`
- 表格列 → 列表项 `view` 内展示字段
- 分页 → `onReachBottom`
...
```html
.modal / .popup → u-popup 或页面内遮罩 + components/
.form-row / .field → view + text(label) + input/picker
.footer-actions → view 底部按钮区
```
...
从列表项 DOM 或 `<thead><th>` 提取 label；**prop 名从接口文档获取**，不从展示文本猜。

### 小程序页识别（C 端原型）

| 特征 | 说明 |
|------|------|
| 底部 `navigation` / Tab | 主 Tab 路由，映射 `pages/index/index` 自定义 tab |
| 手机状态栏 `9:41` | 忽略，非业务字段 |
| `page-spec-panel` | **优先读说明**，再解析 UI |
| 半屏弹层 / 底部工具栏 | `u-popup` 或页面内遮罩 |

---

## 步骤 3：原型 → Vue 映射规则（uni-app）

| 原型 class / 元素 | Vue 实现 |
| ----------------- | -------- |
| `.primary-btn` | `button` 主色 class |
| `.ghost-btn` | `button` plain / ghost |
| `.link` | `text` + `@click` 或 `navigator` |
| `.input` | `input` |
| `.table` / 列表行 | `view` + `v-for` 列表项 |
| `.radio-line` | `radio-group` |
| `.modal` | `u-popup` in `components/` |
| `.screen-title` | `custom-nav` 标题或页面内 `text` |
| `.screen-desc` | 页面说明区 `text` |

**禁止**复制原型 CSS 类名到 Vue；使用 `uni.scss` 变量与 `rpx`。
...
└── components/EditPopup/index.vue
```

接口定义放页面 `services.js`；存量集中式项目则追加到对应 `service/<domain>.js`。
...
3. 列表页骨架（Vue 2 Options API 示例）：

```vue
<template>
  <view class="page">
    <custom-nav title="商品标签" />
    <view class="toolbar">
      <button class="btn-primary" @click="handleCreate">新增标签</button>
    </view>
    <view v-for="item in list" :key="item.id" class="row" @click="handleEdit(item)">
      <text>{{ item.name }}</text>
      <text>{{ item.statusDesc ?? '-' }}</text>
      <text>关联 {{ item.productCount ?? 0 }}</text>
    </view>
    <view v-if="!list.length && !loading" class="empty">暂无数据</view>
    <EditPopup v-if="dialogVisible" :record="currentRow" @success="loadList" @close="dialogVisible = false" />
  </view>
</template>

<script>
import { GetTagList } from './services' // 存量集中式见 project-conventions.md
import EditPopup from './components/EditPopup/index.vue'

export default {
  components: { EditPopup },
  data() {
    return { list: [], loading: false, pageNo: 1, pageSize: 10, next: true, dialogVisible: false, currentRow: null }
  },
  onLoad() { this.loadList() },
  onReachBottom() { if (this.next && !this.loading) this.loadMore() },
  methods: {
    async loadList() {
      this.pageNo = 1
      this.next = true
      this.loading = true
      try {
        const res = await GetTagList({ pageNo: this.pageNo, pageSize: this.pageSize })
        this.list = res.data.data || []
        this.next = this.list.length >= this.pageSize
      } finally { this.loading = false }
    },
    loadMore() { /* 递增 pageNo 追加 list */ },
    handleCreate() { this.currentRow = null; this.dialogVisible = true },
    handleEdit(row) { this.currentRow = row; this.dialogVisible = true },
  },
}
</script>
```

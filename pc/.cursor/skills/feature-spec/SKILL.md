...
5. `.cursor/project-conventions.md`（若存在，对齐布局/request）
...
`<feature-slug>`：kebab-case，建议 `<module>-<page>`，如 `product-tag-list`。
...
**新建页默认路径：** `feature-spec`（propose）→ `feature-dev-workflow`（apply）→ `feature-verify` → `feature-archive`

...
version: 1.0.1
description: 手写接口联调：生成 services/types、字段映射与分页对齐；入参透传、出参原样绑定。 用户说联调、对接接口、字段对一下、联调报错时触发；有 OpenAPI 优先 openapi-api-integration。
---
# 接口对接
...
分页字段与文档不一致时，在 **HiTable `dataFormat`** 或公共 `listRequest` **一处**映射并注释。
...
- ✅ 按接口文档写字段，不改 request 封装
- 🚫 buildXxxQuery 兜底、normalizeRows 猜字段

...
version: 1.0.1
description: 解析 PRD 文案与 UI 截图，提取页面结构、字段、交互与接口规划。 用户说按 PRD 开发、实现需求时触发；Markdown 先走 prd-markdown-ingest。
---
# PRD 驱动的功能开发
...
| 特征                         | 类型          | 代码要点                          |
| ---------------------------- | ------------- | --------------------------------- |
| 筛选区 + 表格 + 分页         | 列表页        | HiTable + filter + tablePage.scss |
| 多字段录入 + 提交            | 表单页        | `表单与详情页开发指南.mdc` + formPage.scss |
| 只读信息分组展示             | 详情页        | `表单与详情页开发指南.mdc` §详情页         |
| 列表 + 弹窗编辑              | 组合页        | index.vue + components/EditDialog |
| 多统计卡片 + 多列表/图表区块 | 工作台/仪表盘 | 见复杂页面开发指南，按区块拆分    |
| 左右/上下分栏，两侧独立操作  | 双栏页        | 侧栏 + 主区各一个 hook            |
| 多 Tab，每 Tab 内容独立      | Tab 组合页    | lazy Tab + 每 Tab 独立子目录      |
...
/** 标签列表 */
export const GetTagList = (data: Record<string, unknown>) => request({ url: 'Product/Tag/List', data, method: 'post' })

/** 保存标签 */
export const SaveTag = (data: Record<string, unknown>) => request({ url: 'Product/Tag/Save', data, method: 'post' })
```
...
- ✅ 产出需求摘要后衔接 feature-dev-workflow
- 🚫 无 PRD 依据编造业务规则

...
version: 1.0.1
description: 对照 e2e.md 在浏览器执行 P0 交互冒烟；优先 Browser MCP，不可用则降级清单。 用户说【verify-e2e】、UI 验收、浏览器回归时触发。
---
# Feature E2E 冒烟验收
...
> **定位：** L3 验收 — 验证**浏览器内**关键路径可点、可渲染、无控制台致命错误。  
> **不替代：** L1 `feature-verify`（SDD/规范/build）、L2 `api-smoke`（接口结构）。  
> **MCP：** 优先 **Browser MCP**；**未就绪时不得伪造 E2E pass**。  
> **探针顺序：** Browser MCP →（Node 修复后重试）→ Playwright（Shell/`npx`）→ skip + 人工清单。
...
**交付首行：** `Feature E2E: <slug> | PASS/FAIL/SKIP | P0 x/y | Browser/Playwright/skip`

**门禁**

- [ ] P0 路径结果表（PASS/FAIL/skip）
